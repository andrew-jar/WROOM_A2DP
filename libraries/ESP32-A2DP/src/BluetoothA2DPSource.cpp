...
void BluetoothA2DPSource::filter_inquiry_scan_result(
  ...
  // MOD AJ: disabled strict RENDERING filter so scan can list all audio-like devices.
  // Oryginał w 1.8.11 sprawdzał valid_cod_services (RENDERING). Teraz akceptujemy RENDERING, AUDIO i Other.
  if (!esp_bt_gap_is_valid_cod(cod)) {
    ESP_LOGI(BT_AV_TAG, "--Compatiblity: Invalid COD - still checking EIR (MOD AJ)");
  } else {
    if (esp_bt_gap_get_cod_srvc(cod) & ESP_BT_COD_SRVC_RENDERING) {
      ESP_LOGI(BT_AV_TAG, "--Compatiblity: RENDERING");
    } else if (esp_bt_gap_get_cod_srvc(cod) & ESP_BT_COD_SRVC_AUDIO) {
      ESP_LOGI(BT_AV_TAG, "--Compatiblity: AUDIO (MOD AJ - accepted)");
    } else {
      ESP_LOGI(BT_AV_TAG, "--Compatiblity: Other COD 0x%x (MOD AJ - accepted)", cod);
    }
  }

  /* search for target device in its Extended Inqury Response */
  if (eir) {
    ESP_LOGI(BT_AV_TAG, "--Compatiblity: Compatible (MOD AJ)");
